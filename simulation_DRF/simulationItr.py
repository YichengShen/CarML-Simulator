...
def simulate(simulation):
    ...
    return cloud_server.combined_rf
...
def main():
    ...
    dataset_to_learn = Training_Dataset(1, X_train, y_train)

    rsu_list = sumo_data.rsuList(RSU_RANGE, NUM_RSU)

    simulation = Simulation(FCD_FILE, vehicle_dict, rsu_list, dataset_to_learn)
    combined_rf = simulate(simulation)

    # Calculate score
    print("rf combined score is", combined_rf.score(X_test, y_test))
